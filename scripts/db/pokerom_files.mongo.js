...
// prettier-ignore
var aggregationsObj = {"Calc Total ROMs Size Bytes":[{"$group":{"_id":null,"total_length":{"$sum":"$length"}}},{"$addFields":{"total_size":{"$toString":{"$toLong":"$total_length"}}}},{"$project":{"_id":0,"total_length":1,"total_size":{"$concat":["$total_size"," ","Bytes"]}}},{"$limit":1}],"Calc Total ROMs Size Gibibytes":[{"$group":{"_id":null,"total_length":{"$sum":"$length"}}},{"$addFields":{"total_size":{"$toString":{"$round":[{"$toDouble":{"$divide":["$total_length",{"$pow":[1024,3]}]}},2]}}}},{"$project":{"_id":0,"total_length":{"$toDecimal":{"$divide":["$total_length",{"$pow":[1024,3]}]}},"total_size":{"$concat":["$total_size"," ","Gibibytes"]}}},{"$limit":1}],"Filter GB ROMs":[{"$sort":{"length":-1,"filename":1,"uploadDate":1}},{"$match":{"filename":{"$regex":"^[\\w\\d\\-_]+\\.gb$","$options":"i"}}}],"Filter 3DS ROMs":[{"$sort":{"length":-1,"filename":1,"uploadDate":1}},{"$match":{"filename":{"$regex":"^[\\w\\d\\-_]+\\.3ds$","$options":"i"}}}],"Filter GBA ROMs":[{"$sort":{"length":-1,"filename":1,"uploadDate":1}},{"$match":{"filename":{"$regex":"^[\\w\\d\\-_]+\\.gba$","$options":"i"}}}],"Filter GBC ROMs":[{"$sort":{"length":-1,"filename":1,"uploadDate":1}},{"$match":{"filename":{"$regex":"^[\\w\\d\\-_]+\\.gbc$","$options":"i"}}}],"Filter NDS ROMs":[{"$sort":{"length":-1,"filename":1,"uploadDate":1}},{"$match":{"filename":{"$regex":"^[\\w\\d\\-_]+\\.nds$","$options":"i"}}}],"Filter XCI ROMs":[{"$sort":{"length":-1,"filename":1,"uploadDate":1}},{"$match":{"filename":{"$regex":"^[\\w\\d\\-_]+\\.xci$","$options":"i"}}}],"Proper Rom Files Sort":[{"$addFields":{"field_length":{"$strLenCP":"$filename"}}},{"$sort":{"length":1,"field_length":1,"uploadDate":-1}},{"$unset":"field_length"}],"Show Rom Sizes (KB)":[{"$addFields":{"rom_size":{"$ceil":{"$divide":["$length",1024]}}}},{"$project":{"filename":1,"length":1,"chunkSize":1,"uploadDate":1,"md5":1,"rom_size":{"$concat":[{"$toString":{"$toLong":"$rom_size"}}," ","KB"]}}}],"Sort ROM Files By Length (Descending)":[{"$sort":{"length":-1}}],"Count ROM Files":[{"$count":"id"},{"$addFields":{"rom_files_count":{"$toInt":"$id"}}},{"$project":{"id":0}}],"rom.files Metadata":[{"$addFields":{"romFileType":{"$split":["$filename","."]}}},{"$project":{"romFileType":{"$toUpper":{"$arrayElemAt":["$romFileType",1]}},"filename":1,"uploadDate":1,"md5":1,"chunkSize":1,"length":1}},{"$addFields":{"metadata":{"romType":"$romFileType","contentType":"application/octet-stream"}}},{"$project":{"romFileType":0}}]};
